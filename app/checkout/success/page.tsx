...
function SuccessContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFDFD] px-6">
        <div className="text-center">
          ...
          <p className="text-[10px]  tracking-[0.3em] text-stone-400 animate-pulse">Loading order details...</p>
        </div>
      </div>
    );
  }
  ...
  if (!reference) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white px-6">
        <div className="text-center">
          <p className="text-[10px]  tracking-[0.3em] text-stone-400 mb-4">No Reference Found</p>
          <Link href="/shop" className="text-sm underline underline-offset-8  tracking-widest text-stone-900">
            Return to Gallery
          </Link>
        </div>
      </div>
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white px-6">
        <div className="text-center">
          <p className="text-[10px]  tracking-[0.3em] text-stone-400 mb-4">Order Not Found</p>
          <Link href="/account" className="text-sm underline underline-offset-8  tracking-widest text-stone-900">
            Go to Account
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#FDFDFD] py-12 px-6 lg:px-12 print:bg-white print:py-0">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-16 print:hidden">
          ...
          <p className="text-[10px]  tracking-[0.6em] text-stone-400 mb-3 font-medium">Order Confirmed</p>
        ...
        <div className="bg-white border border-stone-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] overflow-hidden print:border-stone-200 print:shadow-none">
          <div className="p-8 lg:p-14">
            ...
            <div className="flex justify-between items-start mb-16 border-b border-stone-50 pb-10">
              <div>
                <h2 className="text-xs  tracking-[0.3em] font-bold text-stone-900 mb-2">Your Receipt</h2>
                <p className="text-[10px] text-stone-400  tracking-widest">Transaction Ref: {reference}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px]  tracking-[0.2em] text-stone-400 mb-1">Date of Issue</p>
                <p className="text-xs font-medium text-stone-900  tracking-tighter">{orderDate}</p>
              </div>
            </div>

            {/* Product Table */}
            <div className="space-y-8 mb-16">
              <h3 className="text-[10px]  tracking-[0.4em] text-stone-300 font-bold">Your Selection</h3>
              {order.items.map((item, index) => (
                <div key={index} className="flex justify-between items-center group">
                  <div className="flex items-center gap-6">
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-stone-900  tracking-widest mb-1">{item.name}</p>
                      <p className="text-[10px] text-stone-400  tracking-tighter">
            ...
            <div className="space-y-4 pt-10 border-t border-stone-100">
              <div className="flex justify-between text-[10px] text-stone-400  tracking-[0.2em]">
                <span>Subtotal</span>
              ...
              <div className="flex justify-between items-baseline pt-6">
                <span className="text-xs  tracking-[0.4em] font-black text-stone-900">Amount Paid</span>
          ...
          <div className="bg-stone-50/50 px-8 py-6 flex justify-between items-center print:hidden border-t border-stone-50">
            <div className="flex items-center gap-2 text-[9px] text-stone-400  tracking-widest font-medium">
              <ShoppingBag size={12} /> Verification Successful
            </div>
            <button 
              onClick={handleSaveReceipt}
              className="flex items-center gap-2 text-[10px]  tracking-[0.2em] font-bold text-stone-900 hover:text-stone-500 transition-colors"
            >
        ...
        <div className="mt-16 flex flex-col sm:flex-row gap-6 justify-center items-center print:hidden">
          <Link 
            href="/shop" 
            className="w-full sm:w-auto px-12 py-5 bg-blue-700 text-white text-[10px]  tracking-[0.4em] font-bold text-center hover:bg-blue-800 transition-all flex items-center justify-center gap-3 shadow-lg"
          >
            Continue Browsing <ArrowRight size={14} />
          </Link>
          <button 
            onClick={() => window.print()} 
            className="w-full sm:w-auto px-12 py-5 border border-stone-200 text-stone-500 text-[10px]  tracking-[0.4em] font-bold text-center hover:bg-stone-50 hover:text-stone-900 transition-all flex items-center justify-center gap-3"
          >
        ...
        <div className="hidden print:block mt-32 text-center border-t border-stone-100 pt-10">
          <p className="text-[10px]  tracking-[0.3em] text-stone-400">Thank you for choosing artisanal quality.</p>
          <p className="text-[8px] text-stone-300 mt-4 tracking-widest ">Your Brand Name • Nairobi, Kenya</p>
        </div>
      </div>
